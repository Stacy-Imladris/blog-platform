...
export const getVideosRouter = () => {
    ...
    router.get('/', (req: RequestWithQuery<QueryVideosModel>, res: Response<VideoViewModel[]>) => {
    ...
    router.post('/', titleValidator, inputValidationMiddleware, (req: RequestWithBody<CreateVideoModel>, res: Response<VideoViewModel | APIErrorResultType>) => {
    ...
    router.put('/:id', titleValidator, inputValidationMiddleware, (req: RequestWithParamsAndBody<URIParamsVideoIdModel, UpdateVideoModel>, res) => {
        const {
            title,
            author,
            canBeDownloaded,
        ...
        const isUpdated = videosRepository.updateVideo(+req.params.id, title, author, canBeDownloaded, availableResolutions, publicationDate, minAgeRestriction)
